...
namespace Senparc.AI.AgentKernel.Tests.Handlers
{
    ...
    public class AgentAiHandlerTests: KernelTestBase
    {
        ...
        [TestMethod]
        public async Task ConversationTest()
        {
            AgentAiHandler agentAiHandler = new AgentAiHandler(_senparcAiSetting);
            ...
            AgentSession session =  await iWantToRun.Kernel.ChatClientAgent.CreateSessionAsync();


            var prompts = new[] { "苏州特产有哪些（起您说三个）？", "南京的呢？", "北京的呢？用英文" };
            var i= 0;
            foreach (var prompt in prompts)
            {
                var result = await iWantToRun.RunAsync(prompt, session);
